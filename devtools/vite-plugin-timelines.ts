  ...
  return {
    ...
    configureServer(server) {
      ...
      server.middlewares.use('/api/save-timeline', (req, res) => {
        ...
        req.on('end', () => {
          try {
            const doc = JSON.parse(body) as { id?: unknown; tracks?: unknown; cues?: unknown; duration?: number };
            ...
            if (!doc.id || !doc.tracks || !doc.cues || doc.duration === undefined) {
              res.statusCode = 400;
              res.end('Invalid or missing timeline data');
              return;
            }
