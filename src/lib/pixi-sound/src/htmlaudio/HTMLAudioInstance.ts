...
import type { PlayOptions } from '../Sound';
...
class HTMLAudioInstance extends EventEmitter implements IMediaInstance {
  ...
  public refresh(): void {
    ...
    const soundVolume = (sound?.volume ?? 1) * ((sound?.muted ?? false) ? 0 : 1);
    const instanceVolume = this._volume * (this._muted ? 0 : 1);

    this._source.volume = instanceVolume * globalVolume * soundVolume;

    // Update the speed
    this._source.playbackRate = this._speed * global.speed * (sound?.speed ?? 1);
  }
  ...
  public refreshPaused(): void {
    ...
    const pausedReal = this._paused || (sound?.paused ?? false) || global.paused;
  ...
  public play(options: PlayOptions): void {
    ...
    if (end) {
      ...
      console.assert(end > (start ?? 0), 'End time is before start time');
    }
    ...
    this._source.onloadedmetadata = () => {
      if (this._source) {
        this._source.currentTime = start ?? 0;
        this._source.onloadedmetadata = null;
        this.emit('progress', (start ?? 0) / this._duration, this._duration);
