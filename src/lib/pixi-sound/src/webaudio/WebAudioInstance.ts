...
import type { IMediaInstance } from '../interfaces';
...
class WebAudioInstance extends EventEmitter implements IMediaInstance {
  ...
  public set filters(filters: Filter[]) {
    ...
    this._filters = filters?.length ? filters.slice(0) : [];
    this.refresh();
  }
  ...
  public play(options: PlayOptions): void {
    ...
    const duration: number = this._source.buffer?.duration ?? 0;

    this._duration = duration;
    this._end = end ?? 0;
    this._lastUpdate = this._now();
    this._elapsed = start ?? 0;
    this._source.onended = this._onComplete.bind(this);

    if (this._loop) {
      this._source.loopEnd = end ?? 0;
      this._source.loopStart = start ?? 0;
      this._source.start(0, start);
    } else if (end) {
      this._source.start(0, start ?? 0, end - (start ?? 0));
    } else {
      this._source.start(0, start);
    }
