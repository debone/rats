...
import type { IMediaContext } from './interfaces/IMediaContext';
import type { IMediaInstance } from './interfaces/IMediaInstance';
import { type CompleteCallback, type Options, type PlayOptions, Sound } from './Sound';
...
class SoundLibrary {
   ...
   */
  private _useLegacy: boolean = false;

  /** The global context to use. */
  private _context: IMediaContext = null!;

  /** The WebAudio specific context */
  private _webAudioContext: WebAudioContext = null!;

  /** The HTML Audio (legacy) context. */
  private _htmlAudioContext: HTMLAudioContext = null!;
   ...
   */
  public init(ctx: AudioContext): this {
    if (this.supported) {
      this._webAudioContext = new WebAudioContext(ctx);
    }
   ...
   */
  private _getOptions(
    source: string | ArrayBuffer | AudioBuffer | HTMLAudioElement | Options,
    overrides?: Options,
  ): Options {
    let options: Options;
