...
import type { IMedia, IMediaContext, IMediaInstance } from './interfaces';
import { SoundSprite, type SoundSpriteData, type SoundSprites } from './SoundSprite';
...
class Sound {
  ...
  public static from(source: string | string[] | Options | ArrayBuffer | HTMLAudioElement | AudioBuffer): Sound {
    ...
    const media: IMedia = getInstance().useLegacy ? new HTMLAudioMedia() : new WebAudioMedia();
  ...
  constructor(media: IMedia, options: Options) {
    ...
    this._autoPlayOptions = complete ? { complete } : undefined;
    this.isLoaded = false;
    this._preloadQueue = null;
    this.isPlaying = false;
    this.autoPlay = options.autoPlay ?? false;
    this.singleInstance = options.singleInstance ?? false;
  ...
  public stop(): this {
    if (!this.isPlayable) {
      this.autoPlay = false;
      this._autoPlayOptions = undefined;
...
export { Sound };
export type { CompleteCallback, LoadedCallback, Options, PlayOptions, SoundSpriteDataMap };
