import { CanvasSource, type ICanvas, TextureSource } from 'pixi.js';
 ...
 */
function render(sound: Sound, renderOptions?: RenderOptions): TextureSource {
  const canvas: HTMLCanvasElement = document.createElement('canvas');

  const options = {
    width: renderOptions?.width ?? 512,
    height: renderOptions?.height ?? 128,
    fill: renderOptions?.fill ?? 'black',
    ...renderOptions,
  };
