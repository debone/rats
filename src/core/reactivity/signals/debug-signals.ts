...
const signalDebugRegistry = new WeakMap<Signal<any>, { binding: BindingApi; unsubscribe: () => void }>();

export function registerSignalForDebug<T>(signalInstance: SignalImpl<T>, debugOptions: SignalDebugOptions) {
...
function asyncRegisterSignalForDebug<T>(signalInstance: SignalImpl<T>, debugOptions: SignalDebugOptions) {
      ...
      if (!isReadOnly) {
        try {
          if (signalInstance.computeFn) {
            console.warn(`Cannot set computed signal "${label}" from debug panel.`);
  ...
  try {
    const parentPane = debugOptions.folder || DebugPanel.signalsFolder || DebugPanel.pane;
    ...
    const binding = parentPane.addBinding(debugTarget, 'value', bindingOptions as any);
