 ...
 *   - Or just remove the array deps in the single dependency case?
 * - Batching updates
 *   - Instead of making all changes all times, keep the changes and run only once at the frame
...
export class SignalImpl<T> implements Signal<T> {
  ...
  protected static currentComputation: SignalImpl<any> | null = null;
  protected static computationStack: Set<SignalImpl<any>> = new Set();
  ...
  subscribe(subscriber: Subscriber<T>, immediate = true): Cleanup {
    if (this.disposed) {
    ...
    try {
      ...
      this.subscribers.forEach((subscriber) => {
        try {
          ...
          console.error(`Error in signal subscriber for ${this.getDebugInfo()}:`, error);
        }
      });
    } finally {
...
/*
class EventSignal<T> extends SignalImpl<T> {
  constructor(initialValue: T) {
    super(initialValue);
  }

  set(): void {
    throw new Error('Cannot set value of an event signal directly');
  }
...
// TODO: events (I don't remember why this was a signal)
/*
export function eventSignal<T>(event: GameEvent<T>, debugOptions?: SignalDebugOptions): Signal<T> {
  const instance = new EventSignal(undefined as T);
...
export function getSignalValue<T>(value: SignalValue<T>, defaultValue: T = value as T): NonNullable<T> {
  return (isSignal(value) ? value.get() : (value ?? defaultValue)) as NonNullable<T>;
}
