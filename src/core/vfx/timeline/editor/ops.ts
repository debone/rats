...
export function setKeyValue(doc: TimelineDoc, trackIndex: number, keyIndex: number, value: number | string): TimelineDoc {
  const key = doc.tracks[trackIndex]?.keys[keyIndex];
  if (key) key.value = value;
  return doc;
}

export function setKeyEase(doc: TimelineDoc, trackIndex: number, keyIndex: number, ease: string | undefined): TimelineDoc {
  const key = doc.tracks[trackIndex]?.keys[keyIndex];
  if (!key) return doc;
