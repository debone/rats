...
export interface WaterBottomEntity extends EntityBase<typeof ENTITY_KINDS.waterBottom> {
  bodyId: b2BodyId;
  destroy(): void;
}
...
export const WaterBottom = defineEntity(
  ({ bodyId, onBall, onCheese, onScrap, waterParticles }: WaterBottomProps): WaterBottomEntity => {
    const physics = usePhysics();
    const unmount = getUnmount();
    const worldId = useWorldId();

    const waterBottom: WaterBottomEntity = {
      kind: ENTITY_KINDS.waterBottom,
      bodyId,
      destroy() {
        unmount();
      },
