...
import { attach, onCleanup, type AttachHandle, type EntityBase } from '@/core/entity/scope';
...
import { SequenceDebugSession } from './debug/sequence';
import { followBody } from './actions/follow';
import { emptyTimelineDoc } from './timeline/types';

/**
 * zIndex for emitter containers within the `effects` layer
 */
const EMITTER_Z_INDEX = 1000;

/**
 * Default attach anchor
 */
const ORIGIN: PositionSource = () => ({ x: 0, y: 0 });

/**
 * Hard cap on persistent emitters kept alive at once before priority eviction
 */
const MAX_LIVE_EMITTERS = 24;
const IDLE_TTL_MS = 10_000;

/**
 * Max non-critical `play` calls serviced per frame.
 */
 ...
 * VFX System
 *
 * Manages the lifecycle of visual effects across the game.
 *
...
export class VFXSystem implements System {
  ...
  private readonly onScreenUnloaded = (): void => {
    ...
    this.pool.disposeAll(false);
    this.cooldowns.clear();
    // Disable non-pinned screen filters
  ...
  play<P>(def: BurstDef<P> | SequenceDef<P>, params: P): void | Promise<void> {
    ...
    def.play(params, { emitter, camera: this.context.camera, container: this.layer() });
  }
  ...
  /**
   * One dropdown to pick a sequence + a `seek`/`edit` action pair.
   * `edit` is enabled only for *data-driven* sequences — those with a committed
   * `assets/timelines/<id>.json`, surfaced via the build-time `virtual:timeline-manifest`
   */
  private initSequenceLauncher(folder: FolderApi, seqDefs: SequenceDef<unknown>[]): void {
    ...
    editBtn.on('click', async () => {
      const def = byId.get(state.seq);
      if (!def) return;
      if (!dataDriven.has(def.id)) {
        const res = await fetch('/api/save-timeline', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(emptyTimelineDoc(def.id), null, 2),
        });
        if (!res.ok) {
          console.error(`[VFXSystem] failed to save timeline: ${res.status} ${res.statusText}`);
          return;
        }
        dataDriven.add(def.id);
      }
