...
/**
 * CUTSCENE: Game Over  [sequence]
 *
 * GAME OVER slams in, letters scatter, retry prompt pulses.
 * VFX type: defineSequence — complete choreographed moment with clear start/end.
 */

const gameOverSequence = defineSequence<{ w: number; h: number }>({
  kind: 'sequence',
  id: 'gameOver',
  async build({ w, h }, { layer }) {
    ...
    const retryPulse = animate(retryText, { alpha: [1, 0.15, 1], duration: 900, loop: true });
...
export function gameOver(root: Container, w: number, h: number): () => void {
  ...
  const loop = async () => {
    while (!cancelled) {
      await gameOverSequence.build({ w, h }, ctx);
