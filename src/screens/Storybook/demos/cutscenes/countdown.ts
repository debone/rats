...
/**
 * CUTSCENE: Countdown  [sequence]
 *
 * 3-2-1-GO! countdown with varied number entrance animations and burst particles.
 * VFX type: defineSequence — complete choreographed moment with clear start/end.
 */

const countdownSequence = defineSequence<{ w: number; h: number }>({
  kind: 'sequence',
  id: 'countdown',
...
export function countdown(root: Container, w: number, h: number): () => void {
  ...
  const loop = async () => {
    while (!cancelled) {
      await countdownSequence.build({ w, h }, ctx);
