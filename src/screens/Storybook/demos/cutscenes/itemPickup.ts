...
/**
 * CUTSCENE: Item Pickup  [sequence]
 *
 * A chest opens and an item floats out with sparkles and name reveal.
...
const itemPickupSequence = defineSequence<{ w: number; h: number }>({
  kind: 'sequence',
  id: 'itemPickup',
  async build({ w, h }, { layer }) {
    ...
    clasp.alpha = 0;
    layer.addChild(clasp);

    // Item sprite (hidden until box opens)
    const itemSprite = new Sprite(Assets.get(ASSETS.prototype).textures[item.avatar]);
    ...
    sparkles.x = w / 2;
    sparkles.y = by;
    layer.addChild(sparkles.container);
    ...
    const nameText = new Text({
      text: item.name,
      style: { ...TEXT_STYLE_DEFAULT, fontSize: 9, letterSpacing: 2, fontWeight: 'bold', fill: item.color },
    });
    ...
    // Item idle float
    const baseY = h / 2 - 34;
    let floatCancelled = false;
    const doFloat = async () => {
      while (!floatCancelled) {
        await animate(itemSprite, { y: baseY - 5, duration: 900, ease: 'inOutSine' });
        await animate(itemSprite, { y: baseY, duration: 900, ease: 'inOutSine' });
      }
    };
    doFloat();
    ...
    const hintPulse = animate(hintText, { alpha: [1, 0.2, 1], duration: 900, loop: true });
...
export function itemPickup(root: Container, w: number, h: number): () => void {
  ...
  const loop = async () => {
    while (!cancelled) {
      await itemPickupSequence.build({ w, h }, ctx);
      await new Promise<void>((res) => setTimeout(res, 400));
    }
  };
  loop();
