/**
 * MESH CUTSCENE: Security Camera Alert  [sequence]
 *
...
const dangerScanSequence = defineSequence<{ w: number; h: number }>({
  kind: 'sequence',
  id: 'dangerScan',
...
export function dangerScan(root: Container, w: number, h: number): () => void {
  ...
  const loop = async () => {
    while (!cancelled) {
      await dangerScanSequence.build({ w, h }, ctx);
