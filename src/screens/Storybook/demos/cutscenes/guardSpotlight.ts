...
/**
 * CUTSCENE: Guard Spotlight  [sequence]
 *
 * A sweeping spotlight hunts for a hidden rat in a dark sewer grid.
 * VFX type: defineSequence — complete choreographed moment with clear start/end.
 */

const guardSpotlightSequence = defineSequence<{ w: number; h: number }>({
  kind: 'sequence',
  id: 'guardSpotlight',
...
export function guardSpotlight(root: Container, w: number, h: number): () => void {
  ...
  const loop = async () => {
    while (!cancelled) {
      await guardSpotlightSequence.build({ w, h }, ctx);
      await new Promise<void>((res) => setTimeout(res, 600));
    }
  };
  loop();
