...
/**
 * CUTSCENE: Level Failed  [sequence]
 *
 * Dark overlay, rain, MISSION FAILED drifts down, retry prompt pulses.
 * VFX type: defineSequence — complete choreographed moment with clear start/end.
 */

const levelFailedSequence = defineSequence<{ w: number; h: number }>({
  kind: 'sequence',
  id: 'levelFailed',
  async build({ w, h }, { layer }) {
    ...
    rain.x = w / 2;
    rain.y = 0;
    layer.addChild(rain.container);

    // Horizontal rule lines
    const ruleTop = new Graphics();
    const ruleBot = new Graphics();
    layer.addChild(ruleTop);
    layer.addChild(ruleBot);
    ...
    const retryPulse = animate(retryText, { alpha: [1, 0.2, 1], duration: 1200, loop: true });
...
export function levelFailed(root: Container, w: number, h: number): () => void {
  ...
  const loop = async () => {
    while (!cancelled) {
      await levelFailedSequence.build({ w, h }, ctx);
      await new Promise<void>((res) => setTimeout(res, 400));
    }
  };
  loop();
