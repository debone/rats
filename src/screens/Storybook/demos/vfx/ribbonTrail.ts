/**
 * TECHNIQUE: Trail / Ribbon Renderer — continuous motion trail  (+ screen bloom)
 *
 * Same ribbon polygon technique as swordTrail but driven continuously by
 * the ticker. Two orbs follow phase-shifted Lissajous paths; the ribbon
 * builds behind each one.
 *
 ...
 *
 * Demonstrates the `defineScreen` VFX type: trailBloom wraps a BloomFilter
 * that adds a soft glow pass over the entire layer. Enable/disable in the
 * game via vfx.screen(trailBloom).enable() / .disable().
 */
...
export function ribbonTrail(root: Container, w: number, h: number): () => void {
  ...
  const label = new Text({
    text: 'TRAIL RENDERER — continuous position history  [screen: trailBloom]',
  ...
  const labelB = new Text({
    text: 'trailBloom  ·  defineScreen  ·  BloomFilter applied to entire layer for glow',
    style: { ...TEXT_STYLE_DEFAULT, fontSize: 6, fill: 0x1a2a4a, letterSpacing: 1 },
  });
  labelB.x = 6;
  labelB.y = h - 12;
  root.addChild(labelB);

  // ─── trailBloom screen filter ────────────────────────────────────────────
  // In-game: vfx.screen(trailBloom).enable() applies this to the camera viewport
  ...
  const drawRibbon = (g: Graphics, pts: { x: number; y: number }[], maxW: number, color: number) => {
    ...
    for (let i = 0; i < pts.length; i++) {
      ...
      left.push(p.x + (-dy / len) * hw * 0.5, p.y + (dx / len) * hw * 0.5);
      right.push(p.x - (-dy / len) * hw * 0.5, p.y - (dx / len) * hw * 0.5);
    }
    const poly = [...left];
  ...
  return () => {
    ...
    root.filters = [];
    bloomFilter.destroy();
    [bg, trail1, trail2, orb, label, labelB].forEach((e) => e.destroy());
  };
}
