/**
 * TECHNIQUE: Oriented Sprite — projectile with gravity  [sequence]
 *
 ...
 *
 * VFX type: defineSequence — the arrow launch → arc → impact is a
 * complete choreographed moment with a clear start and end.
 * The storybook loops the sequence automatically.
 */
...
interface Arrow { sprite: Sprite; vx: number; vy: number; active: boolean; }
...
function makeImpactFlash(root: Container, x: number, y: number): () => void {
  ...
  const tick = (dt: { deltaMS: number }) => {
    ...
    ring.clear();
    if (ra > 0) {
      ring.circle(x, y, 4 + rProgress * 26).stroke({ color: 0xffcc44, width: 2, alpha: ra * 0.7 });
      ring.circle(x, y, 2 + rProgress * 14).stroke({ color: 0xffffff, width: 1, alpha: ra * 0.4 });
    }
    if (age > 400) {
      done = true;
...
/** Arrow projectile sequence: launch 4 staggered arrows, wait for all to land. */
const arrowProjectileSequence = defineSequence<{ w: number; h: number }>({
  kind: 'sequence',
  id: 'arrowProjectileDemo',
  build({ w, h }, { layer }) {
      ...
      const tick = (dt: { deltaMS: number }) => {
        ...
        arrows.forEach((a) => {
          ...
          if (a.sprite.y >= groundY) {
            ...
            a.active = false;
            cleanupEffects.push(makeImpactFlash(layer, a.sprite.x, groundY));
          }
          if (a.sprite.x > w + 20) {
            a.active = false;
            cleanupEffects.push(makeImpactFlash(layer, w + 10, a.sprite.y));
          }
        });
        if (arrows.length === LAUNCH_CONFIGS.length && arrows.every((a) => !a.active)) {
          app.ticker.remove(tick);
          setTimeout(() => {
...
export function arrowFlight(root: Container, w: number, h: number): () => void {
  ...
  const label = new Text({
    text: 'ORIENTED SPRITE — rotation tracks velocity (gravity bends the arc)  [sequence]',
  ...
  const labelB = new Text({
    text: 'arrowProjectileSequence  ·  defineSequence  ·  nose dips as vy grows',
  ...
  const loop = async () => {
    while (!cancelled) {
      await arrowProjectileSequence.build({ w, h }, ctx);
    }
  };
  ...
  return () => {
    cancelled = true;
    [bg, ground, label, labelB].forEach((e) => e.destroy());
  };
}
